...
def normalize_wazuh_alert(raw_event: dict[str, Any]) -> NormalizedAlert:
    ...
    return NormalizedAlert(
        ...
        username=(
            ...
            or win_eventdata.get("userContext")
        ),
        subject_username=win_eventdata.get("subjectUserName"),
        target_username=win_eventdata.get("targetUserName"),
        event_id=(
            str(win_system.get("eventID"))
            if win_system.get("eventID") is not None
            else None
        ),
        ...
        task_name=win_eventdata.get("taskName"),
        source_ip=win_eventdata.get("ipAddress"),
        destination_ip=win_eventdata.get("destinationIp"),
